...
FaceModelParameters::FaceModelParameters(std::string root_dir)
{
	...
	init();
	// fs::path root = fs::path("/home/lyr/kidominox/OpenFace-master/build/bin");
	check_model_path(root_dir);
...
void FaceModelParameters::check_model_path(const std::string& root)
{
	...
	else
	{
		...
		std::cout << "Could not find the landmark detection model to load" << "is this "<< std::endl;
	}
}
